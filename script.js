...
document
  .getElementsByClassName("set-time-btn")[0]
  .addEventListener("click", () => {
    if (wake_up_time && lunch_time && nap_time) {
      ...
      if (hr_and_phase == hr_wakeTime) {
        document.getElementById("d-img").style.background =
          'white url("./img/wakeup_image.svg")';
        document.getElementById("img-text").textContent = "wake up!!";
      } else if (hr_and_phase == hr_lunchTime) {
        document.getElementById("d-img").style.background =
          'white url("./img/lunch_image.svg")';
        document.getElementById("img-text").textContent =
          "lets have some lunch";
      } else if (hr_and_phase == hr_napTime) {
        document.getElementById("d-img").style.background =
          ' white url("./img/goodnight_image.svg")';
